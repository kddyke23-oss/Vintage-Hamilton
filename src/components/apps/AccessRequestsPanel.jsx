...
// Modal version of the access-request review flow, embedded directly in the
// Resident Directory's admin toolbar (2026-08-23) so any Directory admin can
// act on requests, not just the site's super admin. Logic mirrors
// src/pages/admin/AccessRequestsPage.jsx — that page is left in place for
// super admins who land on /admin/requests directly.
...
export default function AccessRequestsPanel({ onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 flex items-start justify-center z-[1400] p-4 overflow-y-auto">
      <div className="bg-brand-50 rounded-2xl shadow-2xl w-full max-w-3xl my-8">
        {/* Header */}
        <div className="bg-white rounded-t-2xl px-6 py-4 flex items-center justify-between border-b border-brand-100 sticky top-0 z-10">
          <div>
            <h2 className="font-display text-2xl text-brand-800">Access Requests</h2>
            <p className="text-sm text-brand-500">Review and approve new resident access requests.</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={onClose}
              className="text-brand-400 hover:text-brand-700 text-2xl leading-none px-2"
              aria-label="Close"
            >
              ×
            </button>
