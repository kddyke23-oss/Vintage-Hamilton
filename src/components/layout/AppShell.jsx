...
const navItems = [
  ...
  { label: 'Pickleball',      path: '/apps/pickleball' },
...
export default function AppShell({ children }) {
  const { user, signOut, isAdmin, appAccess, hasAppAccess } = useAuth()
        ...
        */}
        ...
        `}>
          <nav className="p-3 w-52 flex flex-col" style={{ height: 'calc(100vh - 3.5rem)' }}>
            ...
            {/* SC Admin link — budget access holders only */}
            {hasAppAccess('budget') && (
              <div className={`${!isAppAdmin ? 'pt-3 mt-3 border-t border-brand-700' : 'pt-1'}`}>
                <Link
                  to="/apps/budget"
                  className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium transition-colors ${
                    location.pathname === '/apps/budget'
                      ? 'bg-gold-500 text-brand-900'
                      : 'text-brand-400 hover:bg-brand-700 hover:text-white'
                  }`}
                >
