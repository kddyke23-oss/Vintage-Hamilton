...
### 2.9d RCP test-run bugs — found and fixed 2026-09-10
Keith relayed two issues from RCP's (Mariesol's) first real test pass, from the daily-digest confirmation emails:

1. **Resident confirmation email showed no title, just "PRIVATE EVENT."** `notify-clubhouse-resident-status` was pulling the title from `calendar_events.title`, which for a private/not-sure booking is always the masked "Private Event — Name" placeholder shown to everyone (2.9's masking, extended in the `actual_title` work above) — including, wrongly, in the one email that only ever goes to the booking's own owner. **Fixed:** the email now prefers the resident's own `actual_title` (falling back to the masked title if they never set one), the same resolution already used on-screen for the owner's own view of the event.
2. **A private booking was confirmed with no fee due.** Traced to the "Acknowledge — no fee needed" action RCP had available for any `pending_rcp` booking. Every `pending_rcp` booking is private/not-sure by definition, and every clubhouse resource plus the security deposit is priced now (Keith, 2.9c-era decision: "Scenario 5 does not exist, we have priced everything") — so a genuinely fee-free private booking should no longer occur, and the action existing at all let RCP produce exactly that outcome by mistake. **Fixed:** removed the "Acknowledge — no fee needed" action entirely from `ClubhouseReservationsPage.jsx`; only "Acknowledge — fee required" remains for a `pending_rcp` booking. A dismissed escalation can still correctly confirm with no fee (`resolveEscalation`) — that's a *non*-private booking by the time it reaches that action, a different case, untouched by this fix.

**Cleanup still needed:** RCP's test booking that was confirmed via the now-removed action (a Main Clubhouse reservation, `status='confirmed'`, no fees recorded) has no RCP-side UI path to reopen now that the button is gone — it isn't an escalation candidate (only a non-private confirmed booking can be escalated). Needs a one-time manual fix (cancel-and-rebook, or a direct SQL correction) once Keith confirms which row it is — not yet done.

**Deploy note:** both fixes are code-only, no new migration — `git push` for `SocialCalendar.jsx`'s sibling change is not involved here, this is `src/pages/admin/ClubhouseReservationsPage.jsx` (redeploys with the next Vercel push) and `supabase/functions/notify-clubhouse-resident-status` (needs its own `supabase functions deploy notify-clubhouse-resident-status --no-verify-jwt`).
...
- **Resident — booking processed.** Built 2026-09-03 (Keith: "include it in a confirmation email but also... display on screen"). Two forms, same information, fired the moment RCP acknowledges a request or resolves an escalation:
  - **Email** — `supabase/functions/notify-clubhouse-resident-status`, fired from `acknowledgeFeeRequired` and both `resolveEscalation` outcomes in `ClubhouseReservationsPage.jsx` (the `acknowledgeNoFee` action this used to also fire from was removed 2026-09-10 — see 2.9d). If a fee is now due (`pending_payment`): resource/fee/deposit/total-due breakdown, payment deadline, and the check payable-to/mailing-address settings (2.5) — or a "RCP will follow up directly" line if those aren't filled in yet. If no fee is required (`confirmed`, now reachable only via a dismissed escalation): a simple confirmation, no payment section.
...
### 2.15 Contract alignment build — 2026-09-16

Keith had RCP send over the actual, currently-signed "Clubhouse Lease Agreement" (`VI Clubhouse Rental Agreement 2025.pdf`, saved at the project root) — the first time this engagement compared what was built against the real paper contract. Full review and Keith's decisions:

**What already matched:** base Main Clubhouse rent ($150/6hrs) and security deposit ($250) — both already the live defaults.

**Gaps found, and what Keith decided:**
1. **Vacate time / hour cap.** The contract vacates at 11:00 PM (extendable to midnight only with advance written Board approval, never beyond), and its 6-hour figure is a *pricing* boundary (pay for extra hours), not a hard block. What was built was a hardcoded 10:00 PM cutoff with an outright block past 6 hours for private events — didn't match. **Decision:** both become board-editable settings (`clubhouse_latest_vacate_time`, default 11:00 PM; `clubhouse_additional_hour_fee`, starts unpriced) — a booking can't run past 6 hours until the extra-hour fee is set. A new "I need to stay later" checkbox (`wants_late_end`) lets a resident request it; RCP sees it flagged prominently and takes it to the Board offline — it's a visible flag, not a new blocking status, so the normal acknowledge/fee flow proceeds alongside it. Midnight itself isn't a setting — a same-day time picker can't represent going past it anyway.
2. **Cleaning fee.** The contract has a separate cleaning-fee line item. **Decision:** no new fee field — the security deposit's purpose is explicitly extended in the UI/emails to say it also covers cleaning if the space isn't left as required, matching what the contract already allows.
3. **Occupancy cap.** Contract caps Main Clubhouse at 65; nothing enforced this. **Decision:** `guest_count` is now a required field whenever Main Clubhouse or Side Room is selected, validated against `clubhouse_main_max_occupancy` (default 65) and `clubhouse_side_room_max_occupancy` (**starts NULL — Keith still needs to ascertain this number; no cap is enforced for the Side Room until it's set**).
...
**Built:**
- `supabase/migrations/clubhouse_contract_alignment.sql` — the settings/column additions above, plus `fee_additional_hours` (folded into the existing generated `total_due`). **Not yet applied to the live database.**
- `src/pages/admin/ReportsPage.jsx` — Clubhouse Reservation Settings card extended with the four new fields (vacate time as a real time input, additional-hour fee, both occupancy caps).
- `src/components/apps/SocialCalendar.jsx` — booking form rebuilt: guest count + occupancy validation, tables/chairs quantity inputs, the late-end checkbox, the terms + insurance checkboxes (required whenever the booking is private/not-sure), updated fee breakdown (additional-hour line, deposit/cleaning note), and the new vacate-time/extra-hour validation replacing the old hardcoded rule. `ClubhouseReservationPanel` (on-screen resident view) now shows guests/extras/late-end flag and the signed-by-both-parties line.
- `src/pages/admin/ClubhouseReservationsPage.jsx` — RCP queue now shows guest count, extras, insurance confirmation, and a prominent amber flag for a late-end request; the escalation-confirm path (`resolveEscalation`) now also computes the additional-hour fee retroactively for a 'no'-answer booking that gets confirmed private, from its own start/end time.
- `supabase/functions/notify-clubhouse-rcp/index.ts` — new-booking email to RCP now includes guest count, extras, and the late-end flag.
- `supabase/functions/notify-clubhouse-resident-status/index.ts` — resident status email now includes the additional-hour fee line, the deposit/cleaning note, and the signed-by-both-parties line.
- `npx eslint` passes clean on every touched file (checked individually after each change) — no new warnings/errors introduced anywhere in this build.

**Known gap, not addressed in this build:** a booking that starts as `private_event_answer = 'no'` never shows the terms/insurance checkboxes (those only appear for a private/not-sure answer, matching where the fee/deposit workflow already lived). If RCP later escalates it and the committee confirms it actually is private (`resolveEscalation`, 'confirmed_private'), the resident is retroactively charged a fee but was never asked to accept the Agreement or confirm insurance for it. This is a narrow edge case (an escalated booking has to first be *dismissed as not private by the resident's own answer*, then separately flagged and confirmed) — flagged here rather than silently left implicit, in case it comes up in testing.
